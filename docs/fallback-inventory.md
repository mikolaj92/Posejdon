...
| `SpacyDetector` unavailable/model failure no-op | Returns no findings if the optional model is absent or inference fails. | **Promote outside the facade.** It remains a directly instantiated optional detector and has tests documenting unavailable-model behavior and fusion priority. It is not silently installed by `TextAnonymizer`. |
