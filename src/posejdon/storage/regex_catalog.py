...
DEFAULT_ENTITY_TYPES: tuple[tuple[str, str], ...] = (
    ...
    ("IP_ADDRESS", "IP address references"),
    ("VEHICLE_REGISTRATION", "Vehicle registration numbers"),
    ("DEVICE_ID", "Device and host identifiers"),
...
DEFAULT_REGEX_RULES: tuple[tuple[str, str, str, str, float, int, int, str], ...] = (
    ...
    (
        "BANK_ACCOUNT",
        r"\b(?:rachunek(?:\s+bankowy)?|konto|nr\s+konta|numer\s+rachunku)"
        r"\b[:\s#/-]*(?P<entity>(?:PL[\s-]*)?(?:\d[\s-]*){25}\d)\b",
        "upper_alnum",
    ...
    (
        ...
        r"\b(?:pojazd|nr\s+rej\.?|numer\s+rejestracyjny|tablica(?:\s+rejestracyjna)?)"
        r"\b[:\s#/-]*(?P<entity>[A-Z]{1,3}\s?[A-Z0-9]{4,6})\b",
        "upper_alnum",
        "vehicle_registration",
        0.93,
        1,
        20,
        "Vehicle registration with context label",
    ),
    (
        "DEVICE_ID",
        r"\b(?:identyfikator|device|host|urządzenie)\b[:\s#/-]*"
        r"(?P<entity>[A-Za-z][A-Za-z0-9._-]{2,63})\b",
        "identity",
        "device_identifier",
        0.9,
        1,
        25,
        "Device identifier with context label",
    ),
    (
        "SWIFT_BIC",
