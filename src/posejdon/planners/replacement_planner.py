...
class ReplacementPlanner:
    ...
    def _strategy(
        ...
        # Reversible lineage always needs reinjectable placeholders, so masking
        # styles can never apply here regardless of the requested override.
        if processing_mode == ProcessingMode.REVERSIBLE:
            return DeterministicPlaceholderStrategy()
        # An explicit caller override wins over the policy default, including the
        # irreversible category-placeholder -> fixed-mask shortcut below.
